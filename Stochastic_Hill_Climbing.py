...
def bestImprovement():
    ...
    while done == 0:
        ...
        top4 = sorted(range(len(Neighbor_ev)), key=lambda i: Neighbor_ev[i])[-4:]       # getting top 4     
        ...
        if f_curr[0] >= Neighbor_ev[top4[3]]:               #if there were no improving solutions in the neighborhood
            ...
            if rnd > 0.9:
                ...
                x_curr = (Feasible_Neighbor[top4[3]])[:]        #else: move to the neighbor solution and continue
                f_curr = evaluate(x_curr)[:]         #evalute the current solution             
